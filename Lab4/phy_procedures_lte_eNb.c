...
int8_t find_next_ue_index(PHY_VARS_eNB *eNB)
{
  ...
  for (i=0; i<NUMBER_OF_UE_MAX; i++) {
    if (eNB->UE_stats[i].crnti==0) {
    //if ((eNB->dlsch[i]) && (eNB->dlsch[i][0]) && (eNB->dlsch[i][0]->rnti==0)){
...
void prach_procedures(PHY_VARS_eNB *eNB) {
  ...
  uint16_t preamble_max_1st,preamble_energy_max_1st;
  uint16_t preamble_max_2nd,preamble_energy_max_2nd;
  uint16_t i;
  int8_t UE_id;
  int8_t UE_id2;
  ...
  // ====================================================================
  // find the 1st energy and 1st index
  // In this lab, design finding the 2nd energy and 2nd index
  preamble_energy_max_1st = preamble_energy_list[0];
  preamble_max_1st = 0;
  preamble_energy_max_2nd = preamble_energy_list[0];
  preamble_max_2nd = 0;

  for (i=1; i<64; i++) {
    if (preamble_energy_max_1st < preamble_energy_list[i]) {
      preamble_energy_max_1st = preamble_energy_list[i];
      preamble_max_1st = i;
    }
  }

  if(preamble_max_1st==0){
    preamble_energy_max_2nd = preamble_energy_list[1];
    preamble_max_2nd = 1;
  }

  for (int j=1; j<64; j++) {
    if (preamble_energy_max_2nd < preamble_energy_list[j] && j != preamble_max_1st) {
      preamble_energy_max_2nd = preamble_energy_list[j];
      preamble_max_2nd = j;
    }
  }
  // ====================================================================
  ...
  LOG_D(PHY,"[RAPROC] Most likely preamble %d, energy %d dB delay %d\n",
        preamble_max_1st,
        preamble_energy_list[preamble_max_1st],
        preamble_delay_list[preamble_max_1st]);
#endif
  //printf("preamble_energy_max_1st:%d\tnoise_energy+250:%d\n",preamble_energy_max_1st,noise_energy+250);

  //if (preamble_energy_max_1st > 580) {
  if (preamble_energy_max_1st > noise_energy+250) {

	printf("//==================== (index,engery) max 1st: (%d,%d dB), max 2nd: (%d,%d dB), noise: %d dB\n",preamble_max_1st,preamble_energy_max_1st/10,preamble_max_2nd,preamble_energy_max_2nd/10,noise_energy/10);

	// for (i=0; i<64; i++)
	//	printf("engery %d, %d dB\n",i,preamble_energy_list[i]/10);


    UE_id = find_next_ue_index(eNB);

    if (UE_id==0) {
      eNB->UE_stats[(uint32_t)UE_id].UE_timing_offset = preamble_delay_list[preamble_max_1st]&0x1FFF; //limit to 13 (=11+2) bits
      ...
      LOG_D(PHY,"[eNB %d/%d][RAPROC] Frame %d, subframe %d Initiating RA procedure (UE_id %d) with preamble %d, energy %d.%d dB, delay %d\n",
	    ...
	    UE_id,
            preamble_max_1st,
            preamble_energy_max_1st/10,
            preamble_energy_max_1st%10,
            preamble_delay_list[preamble_max_1st]);

      T(T_ENB_PHY_INITIATE_RA_PROCEDURE, T_INT(eNB->Mod_id), T_INT(frame), T_INT(subframe), T_INT(UE_id),
        T_INT(preamble_max_1st), T_INT(preamble_energy_max_1st), T_INT(preamble_delay_list[preamble_max_1st]));
	...
	mac_xface->initiate_ra_proc(eNB->Mod_id,
				    ...
				    frame,
				    preamble_max_1st,
				    preamble_delay_list[preamble_max_1st]*update_TA/update_TA2,
				    0,subframe,0);
      }
