  ...
  int (*allocate_REs)(PHY_VARS_eNB*,
    ...
    allocate_REs = allocate_REs_in_RB;
      ...
      allocate_REs = allocate_REs_in_RB;
        ...
        static int demo = 1;
        if (demo) {
            int offset = frame_parms->first_carrier_offset + (uint32_t)frame_parms->ofdm_symbol_size * (l + (subframe_offset * nsymb));
            printf("[TX DEMO] codeword(modulation symbol)\nl=%d offset: %d\n", l, offset);
            printf("%32s %18s", "antenna port0   |", "antenna port1   ");
            if (frame_parms->nb_antenna_ports_eNB == 4)
                printf("%s%s", "|   antenna port2   |", "   antenna port3\n");
            else
                printf("\n");

            for (int i = 0; i < 12; i++) {
                printf("re index %2d: A0 %5d + %5di | A1 %5d + %5di ", i, ((int16_t *)&(txdataF[0][offset + i]))[0], ((int16_t *)&(txdataF[0][offset + i]))[1], ((int16_t *)&(txdataF[1][offset + i]))[0], ((int16_t *)&(txdataF[1][offset + i]))[1]);
                if (frame_parms->nb_antenna_ports_eNB == 4)
                    printf("| A2 %5d + %5di | A3 %5d +%5di\n", ((int16_t *)&(txdataF[2][offset + i]))[0], ((int16_t *)&(txdataF[2][offset + i]))[1], ((int16_t *)&(txdataF[3][offset + i]))[0], ((int16_t *)&(txdataF[3][offset + i]))[1]);
                else
                    printf("\n");
            }
            demo--;
